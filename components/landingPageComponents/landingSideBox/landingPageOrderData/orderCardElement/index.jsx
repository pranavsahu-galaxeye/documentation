...
const onClickCardContent = (
	...
	if (numberOfPonds !== "Not Available" && currentOrderStatus === "processed" && can_access) {
		// Start loading
		SET_LOADING_SCREEN_BOOLEAN(true);
		// Get the insight id
		const insightId = farmAndPondInfo[0].id;
		...
		navigate(`/region-insight/${orderId}/${insightId}`);
	}
};
...
function OrderCardElement({
	...
	useEffect(() => {
		...
		if (currentOrderStatus === "processing" && createdAt && can_access) {
